...
## Два набора образов: Reality и TLS (автовыбор мастером)

Оригинальный образ `wiktorbgu` работает ТОЛЬКО с ключами **Reality** (есть `pbk`). С ключами **TLS** (и большинством подписок) он падает `invalid public_key`. Поэтому здесь два набора:

| Файлы | Для каких ключей | Что внутри |
|---|---|---|
| `vless-container-<arch>.tar` | **Reality** (security=reality, есть pbk) | оригинальный образ автора |
| `vless-container-tls-<arch>.tar` | **TLS / подписки** (нет pbk) | патченный: reality отключается при пустом public_key, читает переменную ALPN |

**Мастер выбирает образ сам** по типу вставленного ключа и подставляет нужную ссылку `{arch}`. Для TLS-ключа он также резолвит домен сервера в IP (обход DNS-петли внутри туннеля) и добавляет переменную ALPN. Проверено на живом ax² 7.18.2: оба режима поднимают туннель, выход через VPS.

Патч универсален (если Reality-ключ попадёт на патченный образ — reality включится обратно), но по умолчанию Reality-пользователи получают неизменённый оригинал автора.
